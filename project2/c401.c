...
** funkce.
**/
    //hlavny nesmie byt null
    if ((*RootPtr) != NULL)
    {
        //ak pravy nie je null idem ho zrusit
        if ((*RootPtr)->RPtr != NULL)
        {
            BSTDispose(&(*RootPtr)->RPtr);
            //po vynoreni sa musim pozret este do laveho
            if ((*RootPtr)->LPtr != NULL)
                BSTDispose(&(*RootPtr)->LPtr);
        }
        else
            //pravy nebol null tak idem na lavy ak nie je null
            if ((*RootPtr)->LPtr != NULL)
            {
                BSTDispose(&(*RootPtr)->LPtr);
                //po vynoreni sa este musim pozret na pravy
                if ((*RootPtr)->RPtr != NULL)
                     BSTDispose(&(*RootPtr)->RPtr);
            }
    }
